...
public class Level3 extends Level{
    ...
    public Level3(int width, int height){
        super(width, height);
        random = new Random();
        spawnCooldown = random.nextInt(25)+10;
    ...
    public static Level3 getInstance(int width, int height){
        if (instance == null){
            instance = new Level3(width, height);
        }
        return instance;
    }
    ...
    public Enemy createEnemyType(int spawnY) {
        ...
        if (roll % 3 == 0 || roll%2==0) return new HarderEnemy(new Position(width - 6, spawnY));
        else return new AverageEnemy(new Position(width - 6, spawnY));
    }
